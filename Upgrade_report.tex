...
\maketitle % make title without date % 
\tableofcontents
...
This exchange of materials between host and endosymbiont is regulated by an effective biochemical 'bartering' system with numerous feedback cycles (see \fref{fig:summary_from_book}).
...
On top of this system of secretion, uptake and feedback (summarised in \fref{fig:summary_from_book}), there have also been several other observed regulatory interactions between host and endosymbiont.  
...
\begin{figure}
  \begin{center}
    \includegraphics[width=5in]{Chlorella_Paramecium_metabolism_figure.png}
...
The comparison of De-novo transcriptome assemblies can be seen in \fref{tab:assembly_summary}.  
The metrics clearly show the Trinity assembly is far superior to that of the Oases assembly with over a 50\% reduction in total contig number while recovering consistently longer contigs (a histogram of the Trinity contigs lengths and distribution can be see in \fref{fig:contig_lengths}). 
...
\begin{figure}
  \begin{center}
    \includegraphics[width=5in]{contig_lengths.png}
...
The saturation analysis (in which we test whether further sequencing depth would recover more transcripts) is shown in \fref{fig:saturation}.  
...
\begin{figure}
  \begin{center}
    \includegraphics[width=5in]{saturation.png}
...
The results of this analysis were plotted in \fref{fig:phylogenetic_confirmation}.  
...
\begin{figure}
  \begin{center}
    \includegraphics[width=\textwidth]{g9159.png}
...
The median coverage is plotted in \fref{fig:coverage}.  The key concern that this plot raises is that of poor terminal coverage throughout the `Host Bin' and `Endosymbiont Bin'. 
...
\begin{figure}
  \begin{center}
    \includegraphics[width=\textwidth]{coverage_plot_median.png}
...
\begin{figure}
  \begin{center}
    \includegraphics[width=\textwidth]{g5245.png}
...
Using the KEGG Automated Annotated Server KEGG Ontology annotations were recovered for the phylogenetically verified host and endosymbiont sequences and plotted onto the KEGG complete metabolic pathway (see \fref{fig:metabolic_mapping}).  This is a rough preliminary tool that we can use to target later regulatory and interaction investigations for our candidate proteins.

\subsection{Endosymbiont Species Confirmation}

The phylogeny generated using detected Chlorella 18S sequences (see \fref{fig:phylogeny}) within the transcriptome clearly shows the concatenated sequence branching with high basal support (99.1\%) among the clade containing \textit{Chlorella} NC64A and endosymbionts 10 and 12. 
...
\begin{figure}
  \begin{center}
    \includegraphics[height=\textheight]{concatenated_phylogenetic_analysis_of_18s_chlorella_sequences.png}
