...
\textit{Paramecium} are large (\(80-150\mu m\)) phagotrophic single-celled eukaryotes belonging %IS THIS SIZING REASONABLE?
...
and as such have a well-developed theoretical literature and several available genomes (see \ref{}). %Why study section
...
\subsection{\textit{Paramecium bursaria} biology}
...
%\subsection{\textit{Paramecium} genomics}
...
\citep{} %Jennings 1938 + Sonneborn papers before 1938

Unlike all other studied \textit{Paramecium}, \textit{P. bursaria} does not only have 2 mating types and appears to have undergone gene duplication
at the two unlinked mating type loci and different syngens displaying 2, 4 and 8 mating types \citep
Synclonal inheritance - strictly mendelian as supposed to the more complex epigenetic inheritance patterns observed in some members of \textit{P. aurelia} 
complex \citep{Siegel1960,Phadke2009}.

\textit{P. bursaria} reaches sexual maturity after 50-100 fissions \citep{Siegel1960}
Endosymbiotic algae (or other cytoplasmic particles) are not regularly exchanged during mating \citep{Wichterman1946}
Most \textit{Paramecium} have a finite number of vegetative divisions and will die if they do not sexually reproduce \citep{Kiefer2013}.
...
This common RNAi pathway can be invoked by both microinjection of homologous transgenes (transgene-induced silencing) or by feeding \textit{Paramecium} cells
\textit{Escherichia coli} transformed to produce sense and antisense transcripts for the target gene \citep{Galvani2002}
Direct injection of dsRNA into the cytoplasm only triggers transient silencing, likely due to growth related dilution \citep{Galvani2002}.
Heritable silencing cannot be triggered by dsRNA (possibly due to insufficient RDR activity and absence of H3K9 histone factors) in \textit{Paramecium} \citep{Kiefer2013}).
...
\begin{figure}[h!]
    ...
    Figure redrawn and modified from \citep{Kodama2009}.}
        \includegraphics[width=\textwidth]{symbiont_infection.pdf}
\end{figure}
