...
Endosymbiosis is a special case of symbiosis, which is the long-term stable interdependent 
living together (``sym/σύν'' -- together, ``bios/βίωσις'' -- living) of two or more 
organisms to a point of mutual benefit \citep{DeBary1869,Pound1893}.\footnote{Many now expand 
this definition beyond mutualism to include other categories of biological interactions (\cref{tab:biointeractions}) \citep{Leung2008,OMalley2015}}
...
or within the tissues of multicellular organisms.  However, it excludes niches such as the digestive tract of metazoa 
...
\begin{table}[h]
    ...
    \begin{tabular}{|c|c|}
        ...
        \hline
    \end{tabular}
    \caption[Summary of the types of biological interaction]{An overview of the categories of biological interaction and the effect they have on the two interacting
        ...
        growth rate.  Specifically, `\(+\)' represents a positive, `\(-\)' a negative, and `\(0\)' a neutral outcome.
        Therefore, \((+,+)\) indicates an interaction which is beneficial 
        to both partners and \((+, 0)\) one that is beneficial to one partner and neutral to
        the other.
        ...
        In reality most interactions will not fall neatly into one of these categories and throughout its duration
...
degrees of host-symbiont integration, interdependence and ecological interaction types (\cref{tab:biointeractions}).
Even if we restrict ourselves to endosymbioses that are largely ``mutualistic''\footnote{Noting that the exact nature of
a certain endosymbiosis is highly dependent on the specific ecological context at a particular point of time} 
there is a broad range of characteristics.\footnote{Attempting to classify endosymbioses leads us into the territory of a
common motif of biology: the application of discrete schemas to 
continuous distributions of traits.  These biological quantisations are prone to error (fuzzy delineations)
...
the grey areas \citep{Leung2008}.}

For example, in terms of interdependence of host and endosymbiont you can construct a spectrum (\cref{fig:integrationvsdependence})
with ``incidental'' endosymbioses 
such as bacterial or fungal escape of digestion in macrophages at one extreme and at the other obligate systems such as the mitochondria or chloroplast 
where host and symbiont are essentially a single organism \citep{Margulis1998,Archibald2015}.
...
in carefully controlled specialised root nodule structures \citep{Crespi2008}). 
Therefore, a second dimension could be applied to our approximate 
classification of endosymbiosis (\cref{fig:integrationvsdependence}) representing
the degree of host-endosymbiont integration.
Generally interdependence and integration correlate reasonably
well due to the increased selective pressure to minimise lethal aberrant 
interactions that comes with interdependence.  This can be seen 
in the extreme of host-symbiont integration: that of the endosymbiotic organelles, which are so highly integrated they were only conclusively identified as being endosymbiotic
and not part of the cell relatively recently (see \citep{Archibald2014} for a
...
\caption[Spectrum of endosymbiotic relationships]{ 
        Plot demonstrating a fragment of the diversity of endosymbioses and specifically highlighting the possibility of
        ...
        and in terms of life history.  Whereas, interdependence is an approximate measure of the degree to 
...
vacuolar compartments (often from exo- and endocytic systems) e.g. \citep{Kodama2009a} along with a range of 
host oraganelles including the endoplasmic reticulum \citep{Vogt1992}, Golgi body \citep{Cho2011},
mitochondria \citep{Sassera2006}, chloroplast \citep{Wilcox1986} as well as the nucleus 
\citep{Schulz2015}.
...
With this considerable diversity it is perhaps not surprising that endosymbioses have been discovered featuring partners from all three domains of cellular life. %side-steppin the ole viruses thing 
...
For example:\footnote{Although with all these examples it is important not 
...
\begin{itemize}
    ...
    \begin{itemize}
         \item \textit{Hartmannella} and its intranuclear endosymbiont \textit{Candidatus Nucleicultrix amoebiphilia} \citep{Schulz2014}
...
present in the bacterial and archaeal pangenome, especially the only known forms of primary energy
...
Apart from one other putative primary endosymbiosis in \textit{Paulinella chromatophora}\footnote{A euglyphid amoeba with photosynthetic 
...
Secondary endosymbioses are those in which another eukaryotic lineage has engulfed a primary plastid bearing algae before reducing and integrating them into a 
...
These secondary order plastid endosymbioses have occurred independently in divergent eukaryote lineages all over the eToL e.g. chloroarachniophytes and euglenids, and 
an unresolved number of times in the set of cryptomonads, haptophytes, stramenopiles, dinoflagellates and apicomplexans \citep{Keeling2013}.  
Additionally, there are an uncertain number of higher order endosymbioses in the dinoflagellates \citep{Keeling2013}.
...
This means the potential mechanisms through which the endosymbiosis may have originated are likely to be masked.
Facultative systems such as the green algal endosymbionts of \textit{Paramecium bursaria} offer a potential avenue to investigate secondary 
photosynthetic endosymbioses at an earlier stage i.e. before metabolic co-dependence has become fixed (while acknowledging the impossibility of 
...
\textit{Paramecium} and would, initially at least, have also been mixotrophic (combining phagotrophy with phototrophy via the newly acquired plastid \citep{Rockwell2014}) the study of the \textit{Paramecium bursaria}-green algal systems offer a potential insight into this early and fundamental stage of eukaryote evolution.

\section{\textit{Paramecium bursaria}}

\textit{Paramecium} are large (\SIrange{50}{330}{\micro\metre}) phagotrophic single-celled eukaryotes belonging %size from wiki...
to a genetically diverse \citep{Prescott1994} sub-grouping of the alveolates known as the ciliates (see \cref{fig:tol}).
...
see \cref{fig:huygens}) and are some of the longest-standing model unicellular eukaryotes.  They have been used
to study everything from mutagenesis and developmental genetics, to genomics rearrangement and epigenetics \citep{McGrath2014}. 
Therefore, this system has a well-developed methodological \citep{Sonneborn1970} and theoretical literature along with 
several available genomes (see \cref{fig:paramecium_genomes} for information on the genomes and their relative relationship to \textit{P. bursaria}). %Why study section

\textit{Paramecium bursaria} ``the green \textit{Paramecium}'' is distinguished from most\footnote{
    There is at least one other species, \textit{Paramecium chlorelligerum}, that harbours a different green alga (\textit{Meyerella}) \citep{Kreutz2012}
...
\textit{Chlorella}, \textit{Micractinium} and \textit{Coccomyxa}.  
Each \SIrange{100}{160}{\micro\metre}
\textit{P. bursaria} \citep{Jennings1939} cell contains \(\sim300\) endosymbiotic algae maintained in individual perialgal vacuoles (PV) around the cell cortex \citep{Hoshina2009}.
...
\begin{figure}
    \centering
\includegraphics[width=\textwidth]{PBMR_pics.pdf}
    \caption[Light and fluorescent Microscopy of \textit{P. bursaria} and \textit{M. reisseri}]{
        \textbf{A}: Composite fluorescence overlay of \textit{Paramecium bursaria} CCAP 1660/12 captured on an Olympus IX73. \textbf{B, D, and E}: Greyscale light and 
        chlorophyll fluorescence images of \textit{P. bursaria} CCAP 1660/12 captured on a
        Nikon Eclipse 80i.  \textbf{C}: Composite overlay of CCAP 1660/12 captured on an Olympus IX81 microscope.

        All samples were fixed in paraformaldehyde at a concentration of \SI{4}{\percent}.
    }
    \label{fig:pics}
...
beating in co-ordinated metachronal waves \citep{Funfak2015} provide cellular locomotion and. In the case of phagotrophic ciliates like \textit{Paramecium}, these cilia
also play an important role forcing food bacteria towards
...
These are defensive membrane-bound organelles containing a crystalline spike which can be 
rapidly ejected into the environment on fusion of trichocyst membrane with plasma membrane \citep{Hamel2011}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{paramecium_genomes.pdf}
    \caption[Schematic phylogeny of the \textit{Paramecium} genus]{Modified phylogeny redrawn from \citep{Fokin2004,Aury2006,McGrath2014}
...
Canonical stop codons TAA and TAG have been reassigned to produce glutamine therefore there is only one stop codon (TGA) but four glutamine codons \citep{Salim2008}.  
...
degrades and must be reconstituted entirely from the newly formed heterozygous MIC \citep{Jahn2002} (see \cref{fig:pb_sex}).
...
mostly repeat free with small intergenic regions and many short introns (e.g. \SI{25}{\bp} IES elements) \citep{Aury2006}.  
There is also likely to be evidence of at least one whole genome duplication (WGD) (an ancient WGD before the divergence of 
\textit{Tetrahymena} and \textit{Paramecium} clades but not the two most recent WGD (see \cref{fig:paramecium_genomes}) 
...
(weakly correlating with a gene's GC\%, expression level and functional class) \citep{Aury2006,McGrath2014}.
...
improve replication fidelity, a necessary adaptation when maintaining a separate germline \citep{Sung2012}.
...
\begin{figure}
    ...
    \caption[Life cycle of \textit{P. bursaria}]{
        ...
        which triggers two-rounds of meiosis of the MIC with
        ...
        create a syncaryon. Each syncaryon divides once and one product disintegrates before undergoing two subsequent divisions.
...
This is by microinjection
of homologous transgenes (transgene-induced silencing) or by feeding \textit{Paramecium} cells
\textit{Escherichia coli} transformed to produce dsRNA corresponding to the target gene \citep{Galvani2002}.
...
%However, Forward genetic screen experiments conducted by Marker \textit{et. al.} \citep{Marker2014} identified the presence of two distinct
...
%to be essential \citep{Marker2014}.
...
%Rdr2 in specific is necssary for all known siRNA systems as well as sexual reproduction in \textit{P. tetaurelia} \citep{Marker2014}.

%\textit{P. tetaurelia} encodes a large number different core RNAi factors with 8 Dicer or Dicer-like genes, 17 Piwi genes and 4 RdrP genes \citep{Marker2014}.
%At least one genomically directed forward genetic study has been completed in \textit{P. tetaurelia} \citep{Marker2014}.
...
This is no surprise as ciliates have been known to have bacterial \citep{Gortz2009}, archaeal \citep{Wrede2012} and 
...
the phagosomal digestive process \citep{Gortz1988}. 
The \textit{Paramecium bursaria} micronuclei frequently contains bacterial endosymbionts. The closed nature of reproduction
has been suggested as a reason why endonucleobioses are common in \textit{Paramecium} species \citep{Gortz2009}
...
Several of these endosymbionts have been shown to require specific \textit{Paramecium} genes for maintenance \citep{Dohra1998}.
...
%Another key experimental aspect behind the theoretical utility of the \textit{P. bursaria} system to the 
...
\section{\textit{Paramecium bursaria} - green algal endosymbioses}

The \textit{Paramecium}-green algal\footnote{These include \textit{Chlorella}, \textit{Micractinium} and \textit{Coccomyxa} species as mentioned above.
However, historically only \textit{Chlorella} was described and as such most literature refers exclusively to \textit{Chlorella}. Therefore,
\textit{Chlorella} is often best considered as a stand-in term for any of the green algal endosymbionts of \textit{P. bursaria} (see Chapter 3 for
a detailed review and analysis of this)}
 endosymbioses are established when the algae is phagocytosed by the serially phagotrophic \textit{Paramecium} and is then able to escape the digestive vacuole.  
...
This new perialgal vacuole (PV) is released into the cytoplasm and each PV contains an individual algal cell \citep{Kodama2009a}
...
\begin{figure}
    ...
    \caption[Establishment of endosymbiosis in \textit{P. bursaria}]{
        Process by which some endosymbiont escape digestion and generate perialgal valcuoles (PV).  The digestive vacuole (DV)
        transitions through 8 distinct stages during PV differentiation.  Initially a spherical DV-I is formed from the cytopharynx.
        Within a minute acidosomes fuse and differentiate this to the acidic DV-II. Primary lysosome fusion triggers swelling and conversion
        to DV-III.  At this stage endosymbiosis capable endosymbionts escape digestion by an undefined mechanism.  DV-III return to near
        neutral pH and are categorised into 3 categories based on them containing totally bleached, partially bleached or unbleached algae. 
        The DV-III condense into DV-IV following the same total, partial and undigested categories. Finally, DV-IVb undergo budding
        into a mixture of single digested \textit{Chlorella} (SDC) and single green \textit{Chlorella} (SGC).  Finally, SGC translocate 
        to the host cortex and further differentiate into PV \citep{Kodama2009a}.
    Figure redrawn and modified from \citep{Kodama2009a}.}
\end{figure}
As few as a single algal cell can infect the host \citep{Weis1976} however, the majority of algae are digested especially non-competent strains \citep{Kodama2007}. 
Furthermore, it has been established that \textit{Chlorella} strains are fairly host-specific.  
For example, \citet{Summerer2007} showed that \textit{Chlorella} isolated from other ciliates were able to establish endosymbioses with \textit{P. bursaria} however, those isolated from cnidarian \textit{Hydra} were not.  
This paper also demonstrated that \textit{P. bursaria} favours its symbiotic partner over those isolated from other ciliates when given the choice. This suggests specific adaptations have taken place between host and endosymbiont \citep{Summerer2007}.
...
This is largely supported by studies, such as \citet{Karakashian1963}, showing that with a sufficient concentration of bacterial feedstock in the media the growth rate of aposymbiotic \textit{Paramecium} (``bleached'') and \textit{Paramecium} with \textit{Chlorella} endosymbionts are largely equal.
...
Temporary acquisition of phototrophy is estimated by some research \citep{Raven1997} to be less energetically costly than the permanent maintenance of plastids (via endosymbiosis or kleptoplasty) within the host. Therefore, this indicates that this endosymbiosis likely provides other host benefits beyond just the energetics of acquired phototrophy. These include: 
\begin{itemize}
...
\item Protection against predation \citep{Berger1980}. 
    ...
    \item Protection against undesired endosymbionts and/or parasites. Algae in \textit{P. bursaria} form an antagonistic relationship with some 
        bacterial endosymbionts but  %of a un-genetically characterised bacteria is in a russian paper I can't read skobol1982 or something 
        there is experimental evidence that \textit{P. bursaria} can only be infected by bacteria and yeasts after
...
\item Protection against chemical toxins, for example symbiotic \textit{Paramecium} have a much higher survival rate (96\%) to 0.5 mM nickel chloride (NiCl$_{2}$) than aposymbiotic \textit{Paramecium} via an undetermined mechanism \citep{Miwa2009}
...
\begin{itemize}
    ...
    \item Host supplied mono- and divalent cations such as K$^{+}$, Mg$^{2+}$, and Ca$^{2+}$. All of which have key roles in photosynthesis \citep{Kato2009a}.
    \item Protection against \textit{Paramecium bursaria – Chlorella} Virus (PBCV) \citep{Yashchenko2012} a large isocahedral dsDNA, \SI{330}{\kilo\bp} virus with 133-genes that lyses symbiotic \textit{Chlorella} when isolated from the host \citep{VanEtten1983}.  This potentially occurs by preventing contact between PBCV and the endosymbiont.
...
Intriguingly, from experiments using cycloheximide to selectively interrupt endosymbiont but not host protein synthesis it appears that the maltose exporter that is responsible for export of photosynthate from the PV lumen into the host cytoplasm is endosymbiont derived \citep{Muscatine1967}. 
...
This also demonstrates the importance of cytoplasmic localisation and the conditions in the PV to the control of this relationship.
...
The most apparent of these are the synchronising of cell division and circadian rhythms between host and endosymbiont. This integration
is evidenced by the introduction of endosymbiotic \textit{Chlorella} being sufficient to recover a circadian rhythm in arrhythmic \textit{Paramecium} mutants \citep{Miwa2009}.
...
The importance of regulation of endosymbiont distribution at host division is evidenced in the only natural aposymbiotic \textit{P. bursaria} mutant which
has an impairment in this mechanism and thus can't maintain endosymbionts stably \citep{Tonooka2002}. %good material on algae free in first paragraphs-come back to this paper
...
outstanding problems in understanding the evolution of the eukaryotes. \textit{Paramecium bursaria} and its endosymbiosis with green algae such as \textit{Chlorella}
...
prolific endosymbioses.  Furthermore, while there is considerable supporting literature and many established methodological techniques
...
\textit{Chlorella ellipsoidea} \citep{Ris1962}).} it is perhaps apt that further insight may be gleaned by applying the latest modern 
techniques to interrogate their relationship.

Therefore, the main aims of this research are to assess the utility of \textit{P. bursaria} and its algal endosymbionts
as models for the study of the evolution of endosymbiosis. Specifically, are they metabolomically, transcriptomically and/or genomically
tractable? Is this system amenable to ``single cell'' analysis?  Can \textit{M. reisseri} be separated from its \textit{P. bursaria}
as has been observed in other \textit{P. bursaria}-\textit{Chlorella variabilis} strains?  Finally, can RNAi be used to 
test hypotheses generated using these ``omic'' analyses?
