...
X-ray \citep{Wichterman1948}, and cyclohexamide \citep{weis1984effect,Kodama2007}.
Therefore, we attempted 3 of these methods: specifically Paraquat, Cyclohexamide, and 
...
Specifically, filtration using a \(10\mu m\) filter, washing off, resuspension and 3 serial subcultures
...
reverse primer specific to chlorophyte 28S \citep{Hoshina2005} and the ITS4 primer (\(TCCTCCGCTTATTGATATGC\))
...
\begin{figure}
    ...
    \caption{Schematic diagram showing the location of the forward (ITS2-S2F) and reverse
...
formulation.  MrBayes used 2 indepdendent runs of 4 Monte-Carlo Markov-Chains (MCMC) for
...
\begin{itemize}
    ...
    \item Taxonomic assignments were given to the top hits via script
    \item 
\end{itemize}
...
\begin{figure}
    ...
    \caption{Combined MrBayes and RAxML phylogeny of all ITS2 sequences
...
There are a variety of SNPs but never more than a single SNP difference from the basal \textit{M. reisseri}
...
\begin{figure}
    ...
    \caption{Alignment showing the sole SNP (at pos 126 in masked ITS2 alignment)
...
were completed with BayesHammer corrected reads so the difference in perfomance
...
\end{tabular}
\caption{Assembly statistics generated by an analysis of contigs using QUAST. Best values are highlighted in bold.
...
\begin{figure}
    \includegraphics[width=\textwidth]{assemb_gc.pdf}
    \caption{GC densities of the compared genome assemblies.  As expected
...
\begin{figure}
    \includegraphics[width=\textwidth]{assemb_cum_len.pdf}
    \caption{The cumulative length of contigs as a function of
...
\begin{figure}
    \includegraphics[width=\textwidth]{assemb_nx.pdf}
    \caption{The number of X (i.e. gap) in the assembled contigs as a function of their
...
These taxonomic assignments were then used to validate the 34 contig cluters 
...
\begin{figure}
    ...
    This figure highlights how poorly resolved and noisy the decompositon of this single cell
    metagenome.}
...
investigated (e.g. \citep{Hoshina2008,Hoshina2010}) it was proved as unneccessary 
...
SPAdes generates the assembly de-Bruijn using siamese retangular
...
to that of the Q5-SPAdes assembly.   However, on correspondance with the developers
...
consistent in convential non-MDA bulk genomic studies. 
...
could theoretically shed light on the mechanism by which metabolic co-depedence
...
Finally, the \textit{Micractinium reissieri} endosymbiont in cultures CCAP 1660/12 
