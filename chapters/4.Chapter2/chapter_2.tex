...
% what about negative samples - profile them
...
profile was generated for each library.
This was conducted using a custom tool dubbed ``DueyDrop'' which functions as follows.
...
These random samples were subsequently trimmed using a range of quality score thresholds and scoring schemes 
(sliding window and absolute), and minimum length parameters.  A range of trimming tools were also 
...
This will be discussed and explained further in the results/discussion section, however Trimmomatic \citep{Bolger2014a}
...
The trimmed samples were then mapped to the ``baseline'' bulk RNASeq transcriptome reference dataset using bowtie2
...
This ``baseline'' bulk RNAseq consisted of a Trinity \citep{Haas2013} assembly of the light and dark
...
\begin{itemize}
    \item number of paired reads which mapped uniquely to the bulk assembly in a concordant manner (i.e.
        the forward and reverse read mapped within the range of the insert sizes from one another)
    \item number of paired reads which uniquely mapped but in a discordant manner (i.e. both forward and reverse
        reads mapped but were further than the maximum insert size from one another) 
    \item the total number of reads which survived trimming.
\end{itemize}

These values were tallied relative to the statistics from a ``null mapping'' i.e. 
the mapping statistics for the same sampled reads but without any trimming.
...
\subsection{Library screening}
% negatives ??
Using ``DueyDrop'' the taxonomic profile of each library was determined (see \ref{tab:sct_duey} and \ref{tab:bulk_duey}).
As preliminary analyses showed that trimming made minimal impact on the average library taxonomic profile all
...
\begin{table}[h]
     ...
     highlighted in bold were excluded from subsequent analysis.}
 \label{tab:sct_duey}
\end{table}

Forward and reverse reads from the same pair in all SCT libraries displayed the same or very similar taxonomic profiles.
...
Furthermore, Sickle is relatively poorly documented and requires study of the option parsing code to 
...
of cutadapt with that of Trimmomatic.
...
Unfortunately, unlike Trimmomatic, cutadapt requires either manual repair of 
