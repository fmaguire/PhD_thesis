...
relative to bacterial or archael sources is the issue of highly heterozygous polyploid genomes.
...
e.g. Hamming distance (\(D = \sum_{k=0}^{N}\norm{x_{k} - y_{k}}\) for two sequences \(\vec{x}\) and \(\vec{y}\) of length \(N\))
...
possible trees rapidly increases with the number of sequences (for rooted trees \(N_{trees} = \prod_{x=2}^{N_{taxa}} (2x - 3)\)).
