...
All \textit{P. bursaria} cultures were maintained in 
New Cereal Leaf-Prescott Liquid (NCL) medium:
\begin{itemize}
    \item \SI{4.3}{\gram\per\litre} \(CaCl_{2}.2H_{2}O\)
    \item \SI{1.6}{\gram\per\litre} \(KCl\)
    \item \SI{5.1}{\gram\per\litre} \(K_{2}HPO_{4}\)
    \item \SI{2.8}{\gram\per\litre} \(MgSO_{4}.7H_{2}O\)
    \item \SI{1}{\gram\per\litre} wheat bran
\end{itemize} 
NCL medium is gravity filtered via \(GF/C\) paper and autoclaved before use \citep{NCLCCAP}. 
Cultures were stored in 
an incubator at \SI{15}{\degreeCelsius} with a 12:12 light:dark cycle.  The incubator was
lit using 2 \SI{21}{\watt} 865 daylight fluorescent tubes, producing 2000 lumen each.
Cultures were sub-cultured approximately every 2 weeks using fresh NCL medium and were inspected using light microscopy to monitor health.  
...
Another frequently encountered term is that of ``meta-\ldots-omics''.
...
The utility of ``-omic'' approaches is that they allow a researcher to characterise a high proportion 
...
approach such as qPCR would require sequence knowledge to design primers as well as an infeasible amount of
reactions to acquire a characterisation comparable to that obtainable by a transcriptomic approach such as RNAseq. 
...
and at worst generate models that lack any biological relevance \citep{Fang2011}.  
...
hypotheses which can then be tested (and falsified) in isolation using reductionist methods \citep{Casadevall2008}.  
...
has meant the sequencing of cellular transcripts (RNAseq) has largely supplanted microarrays \citep{Wang2009}.
...
transcripts.  However, for bacterial/archaeal work and transcriptomic analyses focusing on non-poly-adenylated
...
Sanger sequencing's main utility lies in high quality short fragment (\SIrange{300}{1000}{\bp}) sequencing to determine or confirm the sequence 
...
These technologies generate huge amounts (on the order of \(10^{6} - 10^{9}\si{\bp}\) of relatively short (on the order of \(10^{1} - 10^{3}\si{\bp}\)) DNA sequences 
...
Ion Torrent's (now Life Technologies) Personal Genome Machine, Applied Biosystem's (now Life Technologies) SOLiD and Illumina's (formerly Solexa) 
HiSeq, MiSeq and older Gene Analyzer II \citep{Nederbragt2013}.

\begin{figure}
    \includegraphics[width=\textwidth]{illumina_lib_prep.pdf}
    \caption[Illumina library preparation overview]{A brief overview of library preparation for Illumina modified from \citep{Mardis2008} and Illumina TruSeq kit documentation. 
    DNA is fragmented before various steps (end-repair and poly-adenylation) to facilitate attachment of adapters. Finally, fragments with adaptors are size-selected
    according to the specification of that particular sequencing run.}
    \label{fig:libprep}
\end{figure}
...
\begin{enumerate}
        \item Library generation: Randomly fragmenting input DNA into short fragments of a specific size
            ...
            each end of a fragment is sequenced separated with a known size unsequenced fragment aiding subsequent assembly (see \cref{fig:libprep})
        \item Clonal amplification: Generation of clonally identical spatially distinct clusters of DNA mainly via emulsion PCR \citep{Dressman2003} (SOLiD, Ion Torrent, 454)
            or bridge PCR \citep{Adessi2000,Fedurco2006} (Illumina) (see \cref{fig:libseq})
...
and the subsequent expansions in the amount of available data (e.g. the US National Center for Biotechnology's (NCBI) short-read archive (SRA)) 
has made both genomic and RNAseq analysis and annotation easier and more effective. 

\begin{figure}
    \includegraphics[width=\textwidth]{illumina_PE_sequencing.pdf}
    \caption[Principles of paired-end sequencing]{A brief overview of paired end sequencing in an Illumina flowcell after library preparation, derived
    from \citep{Mardis2008} and Illumina. Briefly, prepared sequences with adapters 
    are ligated to the flowcell and undergone bridge amplification to generate
    clonal clusters. These clusters are treated to ensure all fragments are in the
    same orientation before iterative sequencing-by-synthesis stages and washes.
    Finally, the clusters are regenerated and cleaved to sequence the opposite
    orientation (i.e. read 2 in paired end sequencing).}
    \label{fig:libseq}
\end{figure}
...
very high throughput (e.g. HiSeq2500 generates up to 400M \SI{125}{\bp} reads per run (\SI{1}{\tera\bp} of data) \citep{Nederbragt2013})
and the lowest cost per \si{\mega\bp} (\(\leq\$0.04\) \citep{Glenn2011}).
...
a potentially bias generating and error-prone amplification step. 
...
Currently there is only one publically available platform: Pacific Biosciences (PacBio) RS platform. PacBio operates on a similar principal
...
(although that is changing as the platforms mature \citep{VanBuren2015}).
...
Therefore, all genomic and transcriptomic sequencing in this thesis has been performed using the 2nd generation
...
All sequencing platforms involve base-calling procedures in which the continuous 
...
mature platforms such as Illumina typically rely on the default
...
such as Conditional Random Fields (CRF) and Recursive Neural Networks (RNN). 
...
as part of the standard sequencing facility diagnostics 
due to their utility in identifying technical problems.
Problems with library preparation and sequencing can be detected by analysis
of various metrics such as the distribution of per-base quality scores across sequences,
distribution of low quality base reads across the flowcell
and the presence of massively over-represented \(k\)-mers and sequences in the raw
libraries.  One of the most commonly used quality control tool is FastQC \citep{fastqc2015}
which performs tests and visualises these metrics and highlights aberrant values.

Identification of potential library problems can be used to attempt to ameliorate
...
with many problems may require re-sequencing.  
Contamination screening
is also a highly important part of sequencing, especially for single cell approaches
...
from 3' to 5': 
\[s(i) = s(i+1)+\phi(i) - Q_{min}\]
where \(\phi\) is a function returning the \(Q\) score for that base and 
\(Q_{min}\) is the minimum defined quality threshold.
These \(s\) scores are then used to determine the start of the trimming point.  
Alternatively,
...
multiple RNAseq and DNAseq datasets and applications (see `File S2' in \citet{DelFabbro2013}). 
Due to its ease of use and maintenance of paired-read correspondence,
...
However, recently, there have been empirical 
studies suggesting the optimal approach is permissive trimming (e.g. \(q \geq 2\)) 
...
coverage by progressively filtering out the most abundant
...
these are reliant on the availability of pre-existing genomic or transcriptomic resources
...
reads. Therefore, assembly is a process of finding a hamiltonian path (HPP)
...
Alternatively, de Bruijn graph assembly involves the decomposition of reads into \(k\)-mer sets.  
...
in the graph (\(NP\)-complete complexity) to that of the 
eulerian cycle (i.e. visit each edge once) problem (EPP) (\(P\) complexity).  
This is a computationally much simpler problem as it has far fewer degrees of freedom.
This is because all edges (and therefore all non-disjoint vertices) must be visited 
whereas in the HPP not all edges will necessarily be used.  
Furthermore, the existence of solution to the EPP within a graph (or subgraph) can be 
easily determined by observing whether that a) the graph is connected and b) the order
of only \(\leq 2\) vertices is odd (i.e. an even number of edges connected to them).
This reduced computational complexity therefore 
allows assembly of much larger datasets \citep{Compeau2011}.
...
paths means greater graph complexity and thus increased risk of error as well as higher
...
One important complication in the assembly of eukaryotic genomes 
relative to bacterial or archaeal sources is the issue of highly heterozygous polyploid genomes.
...
e.g. Platanus. By using a range of approaches such as \(k\)-mer autoextension,
merging haplotypes at both contig assembly and scaffolding steps, and incorporation
of various heuristics in bubble resolution these assemblers
have performed reasonable well on both highly and lowly heterozygous genomes \citep{Bradnam2013,Kajitani2014a}.

Likewise, transcriptome assembly complexity rapidly increases with the number of alleles
expected per gene, ploidy, heterozygosity, presence of 
complex gene families and alternative splicing.
This is particularly problematic in the \textit{PbMr} system owing to the massive
ploidy of the host \textit{Paramecium bursaria} and the numerous whole genome
duplications in its relatively recent evolutionary history \citep{McGrath2014}.  This also 
explains the difficulties in using sister species, as the most sequenced \textit{Paramecium}
...
Machine learning encompasses techniques and methods from various areas including statistics,
optimisation/control engineering, neuroscience and artificial intelligence.
Applications range in complexity from simple linear regression to deep convolutional neural networks (CNN)
...
parameters of a straight line \(h_{\theta}(x) = \vec{\theta}\vec{x}\) (assuming \(x_{0} = 1\) and \(\theta_{0}\) is the
intercept) which minimise the distance 
between the line and the dependent variable (\(y\)). In this case \(x\) and \(y\) 
are most commonly vectors, however, more complicated regression tasks will involve
higher order tensors.
The distance/error is calculated using something known as the cost function e.g. 
for the sum of squares distance measure: \[J(\theta) = \frac{1}{2} m \sum^{m}_{i=1} (h_{\theta}(x_{i}) - y_{i})^2\]
...
partial derivative of \(J\) with respect to \(\theta\): \(\frac{\partial J}{\partial\theta}\)).  
...
been fitted to that dataset, however, it is possible for the model to fit the training
...
high degree polynomial (\cref{fig:fitting}) .  This polynomial will perfectly pass through all the points but is likely to be a worse predictor for the value 
of some new data than a much simpler model which might appear to have a worse
...
Underfitted models will perform poorly on both the training and test data.  

As constantly assessing generalisability using the test data essentially
makes this part of the training data (and thus will lead to overfitting) 
almost all machine learning analyses use the principal of cross-validation.
...
No single model will perform best for all tasks (to paraphrase and simplify Wolpert and McCreedy's ``No Free Lunch Theorem'' \citep{Wolpert1996}),
...
cross-validation is key to generating a useful model.  Another important way to prevent overfitting is to introduce regularisation in the cost function i.e. a term 
which penalises higher model complexity.

\begin{figure}[H]
    \includegraphics[width=\textwidth]{fitting.pdf}
    \caption[Explanation of learning curves and model fitting]{Plot showing a high bias (underfit) model in yellow and a high variance (overfit) model in red.
        ...
        Underfitted models show a very high training error and little difference between test and training data as the model is too simple
...
function. This estimated function \(\hat{y} = \hat{f}(x)\) would then generally
be applied to new data known as the test data for which the expected outputs are not known (i.e. 
\((x_{i}, y_{i}) \not \in \mathcal{D}\)).  In other words, \(i\) samples comprised of \(j\) 
dimensions each can be arranged as an \(i \times j\) matrix \(\mathbfit{X}\) and the outputs
to a vector \(\vec{y}\) of length \(i\). Therefore supervised learning
can be formulated as a means of identifying a mapping (\(\hat{f}\)) between \(\mathbfit{X}\) and \(\vec{y}\):
...
(\(y_{i} \in {\textrm{host}, \textrm{endosymbiont}, ..., C}\)).  These classifications can be binary (two possible outputs i.e. 
...
Generative algorithms, such as Naive Bayes or Restricted Boltzmann Machines (RBMs), 
seek to model the process by which the output data was generated 
from the input i.e. learn the joint probability \(p(x,y)\) and make predictions on that basis via Bayes Theorem:
...
individual classes whereas discriminative models (such as logistic regression/linear classifiers)
would just determine the boundaries between them.
Generative models often perform better on small training sets by preventing overfitting whereas discriminative
classifiers perform better as the training set grows \citep{Ng2002}.

\subsubsection{Support vector machines}

Support Vector Machines (SVMs) are a type of sparse kernel maximum-margin supervised classification algorithm.
With the innovation of the kernel trick in 1992 \citep{Boser1992} and soft-margins in 1993 (not published until \citet{Cortes1995}) SVMs have been 
...
classifiers are typically trained (that is for \(k\) classes, \(k\) SVMs are trained each classifying between label \(k\) and all other labels).
...
\begin{figure}[H]
    ...
    \caption[Maximum margin classification]{A: Demonstration of 3 valid decision boundaries in a 2D classification problem, B: The optimal boundary (H1) is that which maximises
...
the norm of \(\theta\) i.e. \(\norm{\theta}\) but we use \(\frac{1}{2} \norm{\theta}^{2}\) for mathematical convenience. Therefore:

\[argmin_{\theta} \quad J(\theta) = \frac{1}{2} \norm{\theta}^{2} \quad s.t. \quad y_{i}(\theta x_{i}) \geq 1 \quad \forall i\]
...
\[argmin_{\theta} \quad J(\theta) = \frac{1}{2} \norm{\theta}^{2}  + c \sum^{n}_{i=1} \xi_{i} \quad s.t. \quad y_{i}(\theta x_{i}) \geq 1 - \xi_{i} \quad \forall i\]
...
\begin{figure}[H]
    ...
    The dotted line represents a hard-margin classifier which must have a much smaller margin to correctly classify the class 0 outlier. On the otherhand by accepting this misclassification the soft-margin boundary allows a much wider margin.}
    \label{fig:boundary}
\end{figure}
...
data from the input dimensions to a higher dimensional space in which the data is linearly separable.
...
it is done implicitly by computing the inner product of all pairs of data points (\cref{fig:kernel}).  This is a lot more efficient
...
Radial basis function (RBF) is an example of a commonly used kernel (\(\sigma\) 
forming a hyperparamter):
...
using methods like Platt Scaling or the related Relevance Vector Machine (RVM) algorithm.   The other disadvantage is that hyperparameters such as the misclassification
...
of these parameter settings and selecting the best model via cross-validation. However, 
there is both theoretical and empirical evidence that either random search \citep{Bergstra2012} or Bayesian optimisation
\citep{Eggensperger2013} are more efficient means of selecting hyperparameter values. 


\subsection{Unsupervised learning}

The other main form of machine learning is that of unsupervised or descriptive learning.
In which the training dataset has no provided output labels i.e. 
\(\mathcal{D} = \{x_{i} \quad \forall i \in n\}\) and \(y \not \in \mathcal{D}\) (where again \(n\) is the cardinality of 
this training dataset). 
In other words, we just have our dataset and have no additional information.
...
to a lower dimension form \(\mathcal{Y}\) known as a map \(\mathcal{Y} = \{y_{i} \quad \forall i \in n\}\) 
...
and on the map as assessed by metrics such as Kullback-Leibler (KL) divergence \citep{Maaten2008}.
...
\(k\) clusters. It is often used to initialise Gaussian mixture models (GMMs).

Specifically, given a set of \(n\) observations \(x = \{x_{1},...,x_{n}\}\) 
partition each point \((x_{n})\) into \(k\) clusters, where typically \(x\) is a large
matrix.
...
So, with k-means clustering we actually seek the set of \(k\) cluster centroids \(\mu\) 
...
\begin{equation}
    \begin{aligned}
    J = \sum_{i=1}^{n}\sum_{j=1}^{k} = r_{ij} \norm{x_{i} - \mu_{j}}^{2}
    \label{eq:kmeans_cost}
    \end{aligned}
\end{equation}

Therefore, the goal of \(k\)-means clustering is to find values 
for \({r_{nk}}\) and \({\mu_{k}}\) that minimise this linear 
function \cref{eq:kmeans_cost}.
\citep{Bishop2006}

\begin{figure}[H]
    \includegraphics[width=\textwidth]{kmeans.pdf}
    \caption[K-means clustering]{Demonstration of the \(k\)-means algorithm applying 3 rounds of Expectation-Maximisation (EM) to cluster
...
The standard algorithm proceeds in two alternating steps following the initialisation of \(\mu\) with starting
cluster centroid locations \citep{Forgy1965,Lloyd1982}:
\begin{enumerate}
    \item \(argmin_{r_{nk}} J\) i.e. minimise \cref{eq:kmeans_cost}  w.r.t the assignment of points to clusters while keeping
        the cluster centroids fixed.
    \item \(argmin_{\mu} J\) i.e. 
        i.e. minimise \cref{eq:kmeans_cost} w.r.t the position of the cluster centroids while keeping
...
\(\mu\) setting it to 0 and solving for \(\mu\):
\begin{equation}
    \begin{aligned}
        \frac{\partial J}{\partial \mu_{k}} &= 2 \sum_{i=1}^{n} r_{ik} (x_{i} - \mu_{k}) \\
        0 &= 2 \sum_{i=1}^{n} r_{ik} (x_{i} - \mu_{k})\\
        \mu_{k} &= \frac{\sum^{n}_{i=1} r_{ik}x_{i}}{\sum_{i} r_{ik}}\\
    \end{aligned}
\end{equation}


In other words set \(\mu_{k}\) to the mean of all data points \(x_{n}\) assigned io cluster \(k\) (thus \(k\)-means) \citep{Bishop2006}
...
%\(J\) \cref{eq:kmeans_cost} will converge but is liable to get stuck in a local rather than global minimum.
...
other set of discrete data vectors. It has even been applied to fields such as linguistics \citep{Atkinson2005}.
} and the measure of relatedness is some proxy for evolutionary distance ranging from simple distance measures
e.g. the Hamming distance \(d\), which can be defined for 
two sequences (in the form of two vectors \(\vec{x}\) and \(\vec{y}\) of length \(k\)) as the sum
of the number of positions the sequences differ: \[ d = \sum_{i=0}^{k} ( 1 - \delta_{x_{i}, y_{i}} )\]. Note that \(\delta\) is the Kronecker delta function: \[\delta_{ij}= \begin{cases}
	1, &  i = j\\
	0, & i \neq j 
\end{cases} \]
...
possible trees rapidly increases with the number of sequences.
Specifically, for rooted and unrooted trees the number of possible trees \(n\) increases
by the number of taxa \(t\) as follows:
\[\text{For rooted phylogenies}: 
        n = \prod_{i=2}^{t}(3i - 3)\]

\[ \text{For unrooted phylogenies}:
        n = \prod_{i=2}^{t - 1}(3i - 3)
\]
...
horizontal gene transfer has increased it is becoming clear that in some cases
a network like structure may in fact be more appropriate.


Most analyses in this thesis are conducted using amino acid sequences.  
DNA is more likely to display a compositional bias and independence of sites
is often severely violated due to the structure of codons.  
Amino acids have more states so are less susceptible to back mutations
than DNA and are easier to align.  
...
tools such as BLAST and HMMs to attempt to discover
...
Care must also be taken not to unintentionally bias datasets by removing 
...
information to automatically find a set of sequences of a specified cardinality/size
which displays the maximum possible taxonomic diversity for that set size \citep{Zhou2014}.
...
The goal of multiple sequence alignment (MSA) is to align sets sequences such that 
...
This is a non-trivial computational problem which has been proven to have an NP-complete\footnote{A decision problem for which an answer can be verified in polynomial time 
...
rapid identification of homologous regions using Fast Fourier Transforms (FFT) \citep{Katoh2002},
...
model of sequence evolution (see \cref{sec:phyloinf}), all other means of phylogenetic inference do \citep{Le2008}.
...
states and the frequency of each state is equal (i.e. \(p(x \rightarrow y) \quad \forall x \forall y \in {G,C,T,A} \quad \text{where} \quad x\neq y\)
...
sites by using a \(\Gamma\) distribution: \[var_\Gamma = \frac{\alpha}{\beta^{2}}\]
\[\mu_\Gamma = \frac{\alpha}{\beta}\]
with a given shape \(\alpha\) and trivial scale factor \(\beta\) depending on the dataset to scale rates at each site.
...
other has normal substitution rates applied \citep{Hasegawa1985}.\footnote{
    This can also be used with \(\Gamma\) and is approximately equivalent
to the addition of another discrete \(\Gamma\) category.}
...
Another alternative is the CAT model which is a mixture model with 
\(k\) classes each containing a different state frequency.  If \(k=n\) then this is
similar to the aforementioned Bruno's model, however, generally \(k < n\).  A 
 probabilistic process known as a Dirichlet Process Prior (DPP) is used to assign
...
for fixed-effects vs random-effects models \citep{Yang2012}.  However, 
automated partitioning using a DPP has the advantage of not
...
for this is that of the concomitantly variable codon or ``covarion'' model \citep{Penny2001}.
It allows sites to switch between
``on'' and ``off'' across the tree with the proportion of sites in the relative states
determined at each site \citep{Zhou2010}.
%<++> fix citation
...
artefacts such as long-branch attraction (LBA) \citep{Yang1996}.\footnote{
...
Throughout this thesis, I will use two tools which incorporate these various criteria
to infer the best fitting model depending on the input data. 
ProtTest3 \citep{Abascal2005,Darriba2011c} will be used for analyses 
involving protein sequences and jModelTest2 \citep{Posada2008,Darriba2012} for
phylogenetic inference of DNA datasets.

\subsection{Phylogenetic inference}\label{sec:phyloinf}
\subsubsection{Distance and parsimony methods}
...
\begin{enumerate}
    ...
    \item Generate a new distance matrix consisting of the distances of each leaf from this new internal node.
...
distance estimates \citep{Yang2012}.
...
MP has no explicit assumptions relative to other methods however, this also means it is difficult to build in 
...
The popularity of parsimony methods has declined with discoveries that they can produce statistically inconsistent
phylogenies \citep{Felsenstein2001}.  Interestingly, maximum likelihood inference under 
the no common mechanism model (which involves independent branch-length estimation 
for every site and branch) will result in the same phylogeny as parsimony \citep{Tuffley1997}.
Therefore, arguably parsimony can be a special case of likelihood.
...
Expectation-maximisation or the Newton-Raphson method can then be used to find the MLE for branch length and model parameters.
...
In this work, for high-throughput analyses FastTree2 was used due to its considerable 
...
Bayesian inference is, as the name suggests, based on the Bayes theorem 
(\cref{eq:bayes}). Specifically, the most probable tree is recovered
based on the posterior probability:
\[p(\tau, \theta_l, \theta_\mu| D) = \frac{p(\tau, \theta_l, \theta_\mu)p(D|\tau, \theta_l, \theta_\mu)}{p(D)}\]
where \(p(\tau, \theta_l, \theta_\mu)\)
are the prior probability for model parameters (topology \(\tau\), branch length \(\theta_l\), substitution model \(\theta_\mu\)) and \(p(D|\tau, \theta_l, \theta_\mu)\)
is the likelihood of the data given a certain set of parameters with \(p(D)\) as the marginal 
probability.
...
a process known as Markov-Chain Monte-Carlo (MCMC) to sequentially randomly sample
...
new movements that increase the likelihood than decrease it.
...
of branch support \citep{Simmons2003} (although again there has been considerable work comparing
...
in line with the \texttt{C++11} standard \citep{ISOInternationalStandard2011}

The main \texttt{C++} libraries used in addition to the C++11 standard library were:
\begin{itemize}
    \item \texttt{Seqtk} - fastq/a sequence parsing library \citep{SeqtkGitHub}
    \item \texttt{MLPACK} - a high-performance machine learning library \citep{mlpack2013}
    \item \texttt{OpenCV3} - widely used computer vision library \citep{opencv_library}
    \item \texttt{Armadillo} - numerical computation library \citep{Sanderson2010}
\end{itemize}

The majority of tasks were accomplished using the high-level python language (\texttt{python2.7} or \texttt{python3.4}
depending on the application).  In addition to the standard library, the numerical
computation libraries \texttt{numpy} and \texttt{theano}, machine learning library \texttt{scikit-learn},
statistical and scientific libraries \texttt{scipy} and \texttt{pandas}, the bioinformatics libraries
\texttt{scikit-bio}  and \texttt{biopython}, and plotting libraries \texttt{holoviews}, 
\texttt{matplotlib}, and \texttt{seaborn} were all used extensively.
Frequent use was made of literate programming offered by environments such as the \texttt{ipython notebook} (recently
renamed \texttt{jupyter}).

The statistical programming language \texttt{R} \citep{RCoreTeam2015} was used for some data analysis
and visualisation primarily using \texttt{ggplot2} \citep{Wickham2009} and \texttt{dplyr} \citep{Wickham2014}.
This was primarily done using the \texttt{R-Studio} (\url{http://www.rstudio.com/}) integrated development environment) and 
R-markdown \citep{Allaire2014}.

All code was version controlled using \texttt{git} (\url{http://git-scm.com/}) and remotely hosted using \texttt{github} (\url{https://github.com/}) and \texttt{bitbucket} (\url{https://bitbucket.org/})
services.  Unit tests were automatically run on synchronisation (`push') with these remote servers using 
the \texttt{Travis} (\url{https://travis-ci.org/}) Continuous Integration (CI) service.

Incidental scripting was done using \texttt{zsh} and \texttt{bash} languages and all code was written using \texttt{vim} in an \texttt{st} terminal.

\subsection{Hardware}

All analyses were conducted on either the lab cluster (running Ubuntu Server LTS 12.04 and 14.04 (\url{http://www.ubuntu.com}):
\begin{itemize} 
    \item \texttt{PowerEdgeM910} with 2 x Intel Xeon CPU \texttt{E6510 @ 1.73GHz, 512GB} RAM
    \item \texttt{PowerEdgeM910} with 2 x Intel Xeon CPU \texttt{E7-4807 @ 1.87GHz, 512GB} RAM
    \item \texttt{PowerEdgeM620} with 2 x Intel Xeon CPU \texttt{E5-2650 v2 @ 2.60GHz, 512GB}RAM
\end{itemize}
Or on two workstations both running continuously updated versions of \texttt{Arch Linux} (\url{https://www.archlinux.org/}).
\begin{itemize}
    \item Apple MacPro with 2 x Intel Xeon CPU \texttt{E5520 @ 2.27GHz, 16GB} RAM
    \item Dell Precision T7500 with 2 x Intel Xeon \texttt{E5620 @ 2.4GHz, 48GB} RAM
\end{itemize}
