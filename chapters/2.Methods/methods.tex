...
approach such as qPCR or RT-PCR would require sequence knowledge to design primers as well as an infeasible amount of
...
from 3' to 5' e.g. \(s(i) = s(i+1)+\textrm{quality}(i)-Q\) for a minimum quality threshold \(Q\).
...
multiple RNA-Seq and DNA-Seq datasets and applications (see `File S2' in \citep{DelFabbro2013}). 
...
because all edges (and therefore all non-disjoint vertices) must be visited 
whereas in the hamiltonian problem not all edges will necessarily be used.  
Furthermore, the existence of a path within a graph (or subgraph) can be 
...
of various heuristics involving bubble resolution \citep{Bradnam2013,Kajitani2014a}
...
parameters of a straight line \(h_{\vec{\theta}}(\vec{x}) = \vec{\theta} * \vec{x}\) (assuming \(x_{0} = 1\) and \(\theta_{0}\) is the
intercept) which minimise the distance
between the line and the data (for example, the sum of squares distance).
This distance/error is calculated using something known as the cost function e.g. \(J(\vec{\theta}) = \frac{1}{2} m \sum^{m}_{i=1} (h_{\vec{\theta}}(x_{i}) - y_{i})^2\) 
...
how useful your model is likely to be on the test data (and will lead to overfitting) 
therefore almost all machine learning analyses uses the principal of cross-validation.
...
\begin{figure}[H]
    ...
    \caption[Explanation of learning curves and model fitting]{Plot showing a high bias (underfit) model in yellow and a high variance (overfit) model in yellow.
...
function. This estimated function \(\hat{y} = \hat{f}(x)\) (see \cref{eq:sup}) would then generally
...
dimensions each can be arranged as an \(i \times j\) matrix \(\mathbf{X}\) and the outputs
to a vector \(\vec{y}\) of length \(i\). Therefore supervised learning
can be formulated as a means of identifying a mapping between \(\mathbf{X}\) and \(\vec{y}\):
...
from the input i.e. learn the joint probability \(p(x,y)\) and make predictions on that basis via Bayes Theorem (see \cref{eq:bayes}).
...
model the posterior probability \(p(y|x)\)\footnote{strictly
    ...
    as the posterior would be over the parameters i.e. \(p(\mathbf{W}|\mathbf{Y},\mathbf{X})\) (where \(\mathbf{Y}\) and \(\mathbf{X}\) are the whole
dataset and \(\mathbf{W}\) the parameters).}.
...
it is done implicitly by computing the inner product of all pairs of data points.  This is a lot more efficient
...
Radial basis function (RBF) kernel is an example \(K(x_{i}, x_{j}) = exp ( - \frac{\norm{x_{i} - x_{j}}^{2}}{2\sigma^{2}})\) kernel.
...
and on the map as assessed by metrics such as Kullback-Leibler divergence \citep{Maaten2008}.
...
\begin{enumerate}
    ...
    \item \(argmin_{\mu_{k}} J\) i.e. minimise \cref{eq:kmeans_cost} w.r.t the position of the cluster centroids while keeping
...
\footnote{e.g. the Hamming distance \(d\), which can be defined for 
...
However, care must also be taken not to unintentionally bias datasets by removing 
...
Essentially these methods identify and align homologous sequences in 
...
model of sequence evolution\footnote{
Although it is worth noting that maximum likelihood inference under 
...
Therefore, arguably parsimony is a special case of likelihood under this model}
, all other means of phylogenetic inference do \citep{Le2008}.
...
An alternative to this is the CAT model which a mixture model with 
K classes each containing a different state frequency.  If \(K=N\) then this is
similar to Bruno's model, however, generally \(K < N\).  A 
...
\begin{itemize}
    ...
    \item MLPACK - a high-performance machine learning library \citep{mlpack2013}
