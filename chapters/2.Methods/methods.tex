...
\[argmin_{\theta} J(\theta) = \frac{1}{2} ||\theta||^{2}  + C \sum^{n}_{i=1} \xi_{i} s.t. y_{i}(\theta x_{i}) \geq 1 - \xi_{i} \forall i\]
...
which display the maximium possible taxonomic diversity for that set size \citep{Zhou2014}.
...
distribued (i.i.d) \citep{Hasegawa1985}.
...
state frequencies can be defined at each site \citep{Bruno1996} but needs lots of taxa \citep{Lartillot2004}.
