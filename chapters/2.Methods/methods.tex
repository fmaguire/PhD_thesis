...
these are reliant on the existence of pre-existing genomic or transcriptomic resources
...
e.g. Hamming distance (\(D = \sum_{k=0}^{N}|x_{k} - y_{k}|\) for two sequences \(\vec{x}\) and \(\vec{y}\) of length \(N\))
...
``on'' and ``off'' across the tree with te proportion of sites in the relative states
