...
dimensions each can be arranged as an \(i \times j\) matrix \(\bm{X}\) and the outputs
to a vector \(\vec{y}\) of length \(i\). Therefore supervised learning
can be formulated as a means of identifying a mapping between \(\bm{X}\) and \(\vec{y}\):
...
model the posterior probability \(p(y|x)\)\footnote{strictly
    ...
    as the posterior would be over the parameters i.e. \(p(\bm{W}|\bm{Y},\bm{X})\) (where \(bm{Y}\) and \(\bm{X}\) are the whole
dataset and \(\bm{W}\) the parameters).}.
