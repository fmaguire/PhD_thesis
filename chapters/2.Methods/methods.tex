...
parameters of a straight line \(h_{\vec{\theta}}(\vec{x}) = \vec{theta} * \vec{x}\) (assuming \(x_{0} = 1\) and \(\theta_{0}\) is the
intercept) which minimise the distance
between the line and the data (for example, the sum of squares distance).
This distance/error is calculated using something known as the cost function e.g. \(J(\vec{theta}) = \frac{1}{2} m \sum^{m}_{i=1} (h_{\vec{theta}}(x_{i}) - y_{i})^2\) 
...
of the number of positions the sequences differ:
\[
    d = \sum_{i=0}^{k} ( 1 - \delta_{x_{i}, y_{i}} ) 
\]}
