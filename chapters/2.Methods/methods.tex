...
Most algorithms will seek to minimise the value of this cost function \(J(\vec{theta})\) with respect to 
the parameters of the original function \(h_{\vec{theta}}(\vec{x})\).  Typically, this is achieved using a variety of algorithmic optimisation techniques.
The most prevalent of these are gradient descent based methods in which the value of \(\\vec{theta}\) is modified 
in the direction of the gradient of the cost function (determined using the 
partial derivative of \(J\) with respect to \(\vec{theta}\): \(\frac{\partial J_{\vec{\theta}}}{\partial\vec{theta}\)).  
...
from the input i.e. learn the joint probability \(p(x,y)\) and make predictions on that basis via Bayes Theorem (see \ref{eq:bayes}).
